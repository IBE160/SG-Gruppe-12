...
**Document:** `docs/sprint-artifacts/1-4-basic-profile-creation-name-contact-info.context.xml`
...
- Evidence: The `<acceptanceCriteria>` block contains the 5 ACs from the story file, matching the text exactly.
...
- Evidence: The `<interfaces>` block correctly identifies the new `POST /api/v1/profile` and `GET /api/v1/profile` endpoints, and the `updateProfile` service function.

✓ **Constraints include applicable dev rules and patterns**
- Evidence: The `<constraints>` block lists the key constraints (Prisma model extension, protected endpoints, frontend tech stack).
